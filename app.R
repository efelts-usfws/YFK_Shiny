...
ice_runs <- flow.dat |> 
  filter(qualifier == "ICE", year == 2026) |>
  arrange(date) |>
  mutate(
    gap = as.integer(date - lag(date, default = first(date))) > 1,
    run_id = cumsum(coalesce(gap, FALSE))
  ) |> 
  group_by(run_id) |> 
  summarise(
  ...
  mutate(
    start_dummy = as.Date(yday(start) - 1, origin = "1976-01-01"),
    end_dummy   = as.Date(yday(end) - 1, origin = "1976-01-01")
  )
      ...
      flo.plot <- flo.plot +
        geom_rect(
          ...
          aes(
            ...
            xmax = end_dummy,
            ymin = 0,
            ymax = max(flow.dat$mean_discharge, na.rm = TRUE),
            text = "Ice"
          ),
          fill = "lightblue",
          alpha = 0.3,
